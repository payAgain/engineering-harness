...
def main() -> int:
 import argparse
 parser=argparse.ArgumentParser(description="Harness structure check"); parser.add_argument("--root",type=Path,default=Path(__file__).resolve().parents[2]); args=parser.parse_args(); root=args.root.resolve(); problems=harness_check(root)
 if problems:
  for problem in problems: print(problem)
  print("HARNESS_CHECK FAIL: semantic contract violation"); return 1
 print(f"HARNESS_CHECK PASS (level={load_level(root)}, layout=tool-agnostic)"); return 0
